# app/services/openai_service.py - С УЛУЧШЕННЫМ PROMPT ДЛЯ ПОИСКА СДЕЛОК
import httpx
...
class OpenAIService:
    ...
    async def analyze_full_market(self, all_cars: List[Car], brands_stats: Dict[str, List[Car]]) -> Dict[str, Any]:
        """🎯 ГЛАВНЫЙ МЕТОД: Анализ всего рынка с фокусом на поиск лучших сделок"""
        ...
        input_text = self._build_full_market_analysis_input(market_summary,
                                                            all_cars[:30])  # Увеличили до 30 для лучшего поиска
    ...
    def _build_full_market_analysis_input(self, market_summary: Dict[str, Any], sample_cars: List[Car]) -> str:
        """Строит input для полного анализа рынка с фокусом на поиск лучших предложений"""

        system_context = """Ты опытный аналитик автомобильного рынка Кипра с 15-летним опытом и экспертом по поиску выгодных сделок.

ТВОЯ ГЛАВНАЯ ЗАДАЧА: Найти ЛУЧШИЕ ПРЕДЛОЖЕНИЯ в базе данных и объяснить почему они выгодные.
...
🔍 АНАЛИЗ ОПИСАНИЙ - КРИТИЧЕСКИ ВАЖНО ДЛЯ ПОИСКА СДЕЛОК:
- СРОЧНАЯ ПРОДАЖА: "срочно", "urgent", "переезд", "снижена цена", "торг уместен"
- СОСТОЯНИЕ: "отличное", "идеальное", "без вложений", "не требует ремонта" 
- СЕРВИС: "регулярное ТО", "только официальный", "полная история", "все ТО"
- ВЛАДЕНИЕ: "один владелец", "первый владелец", "семейная машина"
- ДОПОЛНЕНИЯ: "полная комплектация", "много опций", "зимняя резина"
- ПРОБЛЕМЫ: "косметический ремонт", "небольшие царапины", "требует вложений"

🎯 КРИТЕРИИ ЛУЧШИХ ПРЕДЛОЖЕНИЙ:
1. Цена ниже рыночной на 10-20%
2. Отличное техническое состояние в описании
3. Надежная модель для климата Кипра
4. Низкие затраты на обслуживание
5. Признаки срочной продажи (переезд, снижение цены)
6. Хорошая история обслуживания
7. Мало владельцев
8. Полная комплектация

ПРИНЦИПЫ АНАЛИЗА:
- Ищи КОНКРЕТНЫЕ преимущества каждой машины
- Обращай внимание на детали в описаниях
- Выявляй скрытые проблемы ("торг", "небольшие вложения")
- Рекомендуй ТОП-10 лучших вариантов с обоснованием"""
        ...
        sample_cars_info = []
        for i, car in enumerate(sample_cars[:30], 1):
            ...
            # ВАЖНО: Полное описание для анализа сделок
            description_text = ""
            if car.description and car.description.strip():
                desc_clean = car.description.strip()[:500]  # Увеличили до 500 символов
                description_text = f"""
📝 ОПИСАНИЕ (КЛЮЧЕВАЯ ИНФОРМАЦИЯ):
{desc_clean}{'...' if len(car.description) > 500 else ''}

🔍 АНАЛИЗИРУЙ ОПИСАНИЕ НА:
- Срочность продажи и причины
- Техническое состояние
- История обслуживания  
- Дополнительное оборудование
- Скрытые проблемы"""
            else:
                description_text = "❌ БЕЗ ОПИСАНИЯ - снижает привлекательность"

            sample_cars_info.append(f"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚗 АВТОМОБИЛЬ #{i} (ID: {car.id}) - ДЕТАЛЬНЫЙ АНАЛИЗ:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📋 ОСНОВНАЯ ИНФОРМАЦИЯ:
• Модель: {car.title}
• Марка: {car.brand} {car.year or 'н/д'} года
• Пробег: {f"{car.mileage:,} км" if car.mileage else 'н/д'}
• Цена: {car.price}{price_euro}
• Место: {car.place}
• Характеристики: {(car.features or 'н/д')[:150]}

{description_text}

💡 ЗАДАЧА: Оцени это предложение как потенциальную СДЕЛКУ!
""")

        return f"""{system_context}

═══════════════════════════════════════════════════════════════
📊 ПОЛНАЯ СВОДКА ПО РЫНКУ - ПОИСК ЛУЧШИХ ПРЕДЛОЖЕНИЙ
═══════════════════════════════════════════════════════════════
...
• Представлено брендов: {market_summary["brands_count"]}
...
═══════════════════════════════════════════════════════════════
🎯 ПРОВЕДИ АНАЛИЗ ДЛЯ ПОИСКА ЛУЧШИХ СДЕЛОК:
═══════════════════════════════════════════════════════════════

**СОСТОЯНИЕ РЫНКА:**
Общая оценка рынка, какие сегменты наиболее привлекательны для покупки

**АНАЛИЗ ПО БРЕНДАМ:**
Для каждого бренда оцени:
- Представленность и ценовая политика
- Надежность моделей для Кипра
- Типичные затраты на обслуживание
- Рекомендации по годам и моделям

**🏆 ТОП-10 ЛУЧШИХ ПРЕДЛОЖЕНИЙ:**
Выбери 10 самых выгодных машин, ОБЯЗАТЕЛЬНО анализируя:

Для каждой машины:
1. Почему это хорошая сделка? (цена, состояние, срочность)
2. Что говорит описание о состоянии и причинах продажи?
3. Потенциальные затраты на обслуживание
4. Риски и на что обратить внимание
5. Финальная рекомендация (ПОКУПАТЬ/ТОРГОВАТЬСЯ/ОСМОТРЕТЬ)

**📊 АНАЛИЗ ОПИСАНИЙ - КЛЮЧЕВЫЕ НАХОДКИ:**
Какие машины имеют признаки:
- Срочной продажи (переезд, снижение цены)
- Отличного состояния
- Хорошей сервисной истории
- Скрытых проблем

**🎯 РЕКОМЕНДАЦИИ ПОКУПАТЕЛЯМ:**
Советы по выбору и стратегии покупки на текущем рынке

ВАЖНО: Указывай конкретные ID машин в рекомендациях!
Фокусируйся на РЕАЛЬНЫХ ПРЕИМУЩЕСТВАХ каждого предложения!"""
