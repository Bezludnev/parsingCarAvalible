...
<body>
    <div class="container">
        <div class="header">
            <h1>🤖 {title}</h1>
            <p>Полный AI анализ автомобилей • {datetime.now().strftime("%d.%m.%Y %H:%M")}</p>
        </div>
        ...
        <div class="footer">
            <p>
                ...
                <strong>Система:</strong> Car Monitor Bot v2.0 |
...
"""
    ...
    def _generate_cars_table_rows(self, cars_data: List[Dict], recommended_ids: List[int]) -> str:
        """Генерирует строки таблицы с машинами"""

        if not cars_data:
            return "<tr><td colspan='8'>Нет данных о машинах</td></tr>"
        ...
        for car in cars_data:
            ...
            title = car.get("title", "N/A")[:50] + ("..." if len(car.get("title", "")) > 50 else "")
